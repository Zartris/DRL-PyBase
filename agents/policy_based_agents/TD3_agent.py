...
from models.model import TwinCritic, TD3Actor
...
class TD3Agent(AgentBase):
    ...
    def step(self, states, actions, rewards, next_states, dones):
        ...
        for state, action, reward, next_state, done in zip(states, actions, rewards, next_states, dones):
            self.memory.add(state, action, reward, next_state, done, agent_idx)
            agent_idx += 1
    ...
    def save_all(self):
        super().save("actor", self.actor)
        super().save("actor_target", self.actor_target)
        super().save("twin_critic", self.twin_critic)
        super().save("twin_critic_target", self.twin_critic_target)
        super().save_stats("state_normalizer")

    def load_all(self, load_path: Path = None):
        self.actor.load_state_dict(super().load_state_dict("actor", load_path))
        self.actor_target.load_state_dict(super().load_state_dict("actor_target", load_path))
        self.twin_critic.load_state_dict(super().load_state_dict("twin_critic", load_path))
        self.twin_critic_target.load_state_dict(super().load_state_dict("twin_critic_target", load_path))
